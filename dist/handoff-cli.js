...
import { ackHandoff, listHandoffs, listInbox, readHandoff, writeHandoff } from './handoff.js';
...
export function markAnnounced(dataDir, sessionId, stamps) {
    if (!sessionId || !stamps.length)
        return;
...
export function announceNew(dataDir, lane, sessionId, now = new Date()) {
    const seen = readAnnounced(dataDir, sessionId);
...
function inbox(dataDir, argv, out) {
...
function handoff(dataDir, argv, out, err) {
...
export async function main(argv, out = s => process.stdout.write(s), err = s => process.stderr.write(s)) {
    ...
    if (sub === 'inbox')
        return inbox(dataDir, rest, out);
    if (sub === 'handoff')
        return handoff(dataDir, rest, out, err);
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
    main(process.argv.slice(2))
        ...
        .catch(error => {
        process.stderr.write(`przm-memory handoffs: ${error.message ?? error}\n`);
        process.exit(1);
    });
}
