...
import { writeHandoff, readHandoff, listHandoffs, listInbox, ackHandoff, type HandoffScope } from './handoff.js';
...
function handoffScope(lane: string | undefined, all: boolean | undefined, withProject: boolean): HandoffScope {
  if (all) return { all: true };
  if (lane) return { lane: normalizeLane(lane) };
...
server.registerTool(
  ...
  async ({ stamp }) => {
    ...
    if (!note) return json({ acked: false, message: `No handoff ${stamp} is addressed to lane ${sessionLane}.` });
